 ...
 * @param {any} $http
 */
function compare_result($scope, $rootScope, $http, interface_url, result_url) {
...
// 获取列表接口
    ...
    $scope.getResultDetail = function (id) {
        $http({
            'method': 'GET',
            'url': interface_url + '/cmc/result/' + id + '/',
            'headers': {
                'Authorization': 'Token ' + localStorage.getItem('auth_token')
            }
        }).success(function (data) {
...
angular
    .module('inspinia')
    .controller('compare_result', compare_result)    // 代码管理==对比结果
