 ...
 * @param interface_url
 */
function bussiness_Job($scope, $rootScope, $http, interface_url) {
    // 获取产品==zxh
    $scope.productList = [];
    ...
    // 获取系统列表==bug134==zxh==0413
    $scope.systemList = [];
    $scope.getSystemList = function () {
        if ($scope.systemList.length === 0) {
            $http({
                'method': 'GET',
                'url': interface_url + '/uasc/system/search/',
                'headers': {
                    'Authorization': 'Token ' + localStorage.getItem('auth_token')
                }
            }).success(function (data) {
    ...
    $scope.addJobConstructConfirm = function () {
        ...
        if (!$scope.params.job_name) {
            $rootScope.showToast('请输入项目名称') ;
            return;
        } else{
            var nameReg= /^[^\u4E00-\u9FA5]+$/;
            if (nameReg.test($scope.params.job_name)) {
            }else {
                $rootScope.showToast('项目名称只允许输入数字、英文和字符');
                return;
            }
        }
        ...
        var params = {
            ...
            env: $scope.params.env?$scope.params.env.id:'',
    ...
    $scope.getJobConstructDetail = function (obj, itemId, callback) {
        $http({
            'method': 'GET',
            'url': interface_url + '/uasc/job/' + itemId + '/',
            'headers': {
                'Authorization': 'Token ' + localStorage.getItem('auth_token')
            }
        }).success(function (data) {
            if(data.code === 403 || data.code === '403'){
                ...
                if ($scope.dialog.status === 'see' || $scope.dialog.status === 'modify') {
                    ...
                    for (var i = 0; i < $scope.envList.length; i++) {
                        if ($scope.envList[i].id === data.result.env) {
    ...
    $scope.editJobConstructConfirm = function () {
        ...
        var params = {
            ...
            name: $scope.params.name,
            job_name: $scope.params.job_name,
            system: $scope.params.system.uuid,
            env: $scope.params.env.id,
        ...
        $http({
            'method': 'POST',
            'url': interface_url + '/uasc/job/' + $scope.editJobConstructId +'/',
            'data': params,
            'headers': {
                'Authorization': 'Token ' + localStorage.getItem('auth_token')
            }
    ...
    $scope.delJobConstructConfirm = function () {
        $http({
            'method': 'DELETE',
            'url': interface_url + '/uasc/job/' + $scope.delJobConstructId + '/',
            'headers': {
                'Authorization': 'Token ' + localStorage.getItem('auth_token')
            }
        }).success(function (data) {
    ...
    $scope.disableJobConstructConfirm = function () {
        $http({
            'method': 'POST',
            'url': interface_url + '/uasc/job/' + $scope.disableJobConstruct + '/',
            'data': {
                'is_active': false
            },
            'headers': {
...
angular
    .module('inspinia')
    .controller('bussiness_Job', bussiness_Job);    // 统一部署----->构建Job
